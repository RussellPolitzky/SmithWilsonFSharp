...
type CashFlowMatrixRow = { MarketPrice:float; Flows:list<(float * float)> }

// Gets the market price and flows for use 
// in the cash flow matrix.
let getFlowsForQuote (basis:int) (quote:Quote) (baseDate:DateTime) =
    ...
    | Swap(sd)    -> let N           = 1.0 // Notional is 1
                     ...
                     let yearFractions = 
                         [ 0..swapPeriods ] 
                         |> List.map (fun index -> (Month(-(swapPeriods-index)*3)).date(endDate))
                         |> Seq.pairwise
                         |> Seq.map (fun (d1, d2) -> getτ d2, getΔ d1 d2)
                     let flows = yearFractions 
                                 |> Seq.map (fun (τ, Δ) -> τ, N*Δ*fixedRate) // Nominal is 1 
                                 |> List.ofSeq   
                     let (τ, flow) = flows.[flows.Length-1]  
                     let last = (τ, N + flow)
                     let flowsWithNominalRedemption = 
                         Array.concat [ (flows |> Array.ofList).[0..swapPeriods-2]; [| last |] ]
                         |> List.ofArray
                     { MarketPrice = 1.0; Flows = flowsWithNominalRedemption }